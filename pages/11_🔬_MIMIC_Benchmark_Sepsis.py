...
st.markdown("Avaliação diagnóstica em larga escala conforme as diretrizes **Sepsis-3** e metodologia **PhysioNet / Harvard-MIT**.")

if not st.session_state.get("authenticated", False):
    st.warning("⚠️ Autentique-se na tela inicial para executar a auditoria.")
    st.stop()
...
with c1:
    ...
    n_pacientes = st.slider("Tamanho da Coorte", 100, 1000, 300, 50)
    btn_run = st.button("🚀 Executar Benchmark Clínico", use_container_width=True)

with c2:
    if btn_run:
        with st.spinner("Processando registros de UTI..."):
            ...
            with col_g1:
                ...
                tpr_sofa = fpr**0.7
                ax.plot(fpr, tpr_pinn, color='#2ecc71', lw=2.5, label="SGP-PINN")
